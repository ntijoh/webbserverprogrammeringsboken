...
När en Sinatra-applikation tar emot en request från en klient söker den efter en _route_ som matchar requestens _verb_ (`GET` eller `POST`) och dess resurst (t.ex `/fruits`). Koden i första routen som matchar kommer att köras, och resultatet läggs i ett `HTTP Response` och skickas tillbaks till klienten som skickat Requesten.
...
En dynamisk route skapas genom att man ersätter den bit av routen som ska vara dynamisk med ett kolon (`:`) följt av en beskrivande ord.
